...
<div class="container mt-5 text-center">
    <h1 style="color: #FFD700;">O seu signo é:</h1>
    ...
    foreach ($signos->signo as $signo) {
        ...
        if ($data_inicio > $data_fim) {
            ...
            if (($data_nascimento >= $data_inicio && $data_nascimento <= new DateTime($data_nascimento->format('Y') . '-12-31')) ||
                ($data_nascimento >= new DateTime($data_nascimento->format('Y') . '-01-01') && $data_nascimento <= $data_fim)) {
                echo "<h2 style='color: #ffffff;'>{$signo->signoNome}</h2>";
    ...
    if (!$signo_encontrado) {
        echo "<p style='color: #ffffff;'>Não foi possível determinar seu signo. Verifique a data informada.</p>";
    }
